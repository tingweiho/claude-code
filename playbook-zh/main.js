...
(function () {
  ...
  // Floating left-rail table of contents — built from the nav dots so it
  // always stays in sync (one top-level entry per module). Lives in #toc.
  const tocEl    = $('#toc');
  const tocLinks = [];
  if (tocEl) {
    navDots.forEach(dot => {
      ...
      a.href = '#' + dot.dataset.target;
      a.dataset.target = dot.dataset.target;
      a.textContent = dot.dataset.tooltip || dot.getAttribute('aria-label') || '';
  ...
  function updateNavDots() {
    ...
    modules.forEach((mod, i) => {
      ...
      const isCurrent = scrollMid >= top && scrollMid < bottom;
      if (dot) {
        if (isCurrent) {
