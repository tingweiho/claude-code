...
**For each module, write a brief to `~/.claude/<course-name>/briefs/0N-slug.md` containing:**
...
**Step 1 (both paths): Setup** — Create the course directory at `~/.claude/<course-name>/` (default). If the user specifies a different output path, use that instead. Copy these four files verbatim using Read + Write (do not regenerate their contents):
...
The course follows a **warm editorial "runbook" style** (modelled on Tingwei's `mrr_migration_runbook.html` — NOT Aircall brand; `aircall-slides` stays Aircall-brand). Read `references/design-system.md` for the tokens, but here are the non-negotiable principles:

- **Warm cream palette**: cream backgrounds (`#faf7f0` / `#f0ece1`), Ink text (`#002620`, never pure black), **terracotta accent** (`#b8421a`), with a **teal "teach"** secondary (`#2a6c8f`) for definitions / info panels.
- **Runbook typography (real weights OK)**: **Fraunces** for headings + italic captions (weights 400–700), **Inter** for body/UI (400; `<strong>` = 600), JetBrains Mono for code/labels/calculations. NEVER Roboto, Arial, Bricolage, or system fonts as display.
- **Generous whitespace**: Modules breathe. Max 3-4 short paragraphs per screen.
- **Alternating backgrounds**: Even/odd modules alternate between the two cream tones for visual rhythm
- **Dark code blocks**: IDE-style with Catppuccin-inspired syntax highlighting on `#011310`
- **Depth without harshness**: Subtle warm-tinted shadows, never black drop shadows
- **Editorial components**: use `.callout` / `.callout-teach` (info panels), `.caption` (italic Fraunces under figures/tables/code), and `.footnote` (muted, top-bordered) — these are the runbook patterns Tingwei likes.
- **Identifiers always in code font**: any field, column, model, table, function, statement, file, or config name must be wrapped in inline `<code>`, never left in body text — e.g. `cost_usd`, `marketing_digital_costs`, `coalesce()`, `clean_*`. `styles.css` renders these as a terracotta chip (monospace, faint rust background, rounded). Applies in prose, callouts, quiz text, step cards, captions, and tooltips — not just code blocks. In Markdown briefs use backticks.
- **Inline calculations get a chip too**: wrap small worked numbers in `<span class="calc">…</span>` so they read as a derivation, not prose — e.g. <span class="calc">3 × $48 = $144</span>. (Learned from the mrr runbook: code-identifier chip = accent; calculation chip = neutral cream with a thin border.) Use `<code>` for identifiers, `.calc` for arithmetic.
